...
namespace TourAgencyView
{
    public partial class FormVouchers : Form
    {
        ...
        public FormVouchers(IVoucherLogic logic)
        {
            InitializeComponent();
            this.logic = logic;
        }
        ...
        {
            if (dataGridViewVouchers.SelectedRows.Count == 1)
            {
                var form = Container.Resolve<FormVoucher>();
                form.Id = Convert.ToInt32(dataGridViewVouchers.SelectedRows[0].Cells[0].Value);
                if (form.ShowDialog() == DialogResult.OK)
                {
                    LoadData();
                }
        ...
        private void buttonDel_Click(object sender, EventArgs e)
        {
            if (dataGridViewVouchers.SelectedRows.Count == 1)
            {
               ...
               MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    int id = Convert.ToInt32(dataGridViewVouchers.SelectedRows[0].Cells[0].Value);
                    try
                    {
                        logic.Delete(new VoucherBindingModel { Id = id });
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show(ex.Message, "Ошибка", MessageBoxButtons.OK,
